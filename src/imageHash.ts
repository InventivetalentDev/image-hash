...
const processPNG = (data, bits, precise, cb) => {
    try {
        const png = PNG.sync.read(data);
        const res = blockhash(png, bits, precise ? 2 : 1);
        cb(null, res);
    } catch (e) {
        cb(e);
    }
...
const processJPG = (data, bits, precise, cb) => {
    try {
        const decoded = jpeg.decode(data);
        const res = blockhash(decoded, bits, precise ? 2 : 1);
        cb(null, res);
    } catch (e) {
        cb(e);
    }
...
export const imageHash = (oldSrc: string | UrlRequestObject | BufferObject, bits: number, precise: boolean, cb: (err?: any, hash?: string) => void) => {
    const src = oldSrc;

    const getFileType = async (data: BufferObject | Buffer | string) => {
    ...
    const checkFileType = (name: string | undefined, data: BufferObject | Buffer | string) => {
        getFileType(data)
            .then((type) => {
                ...
                if (name && name.lastIndexOf('.') > 0) {
                    ...
                    if (ext === 'png' && type.mime === 'image/png') {
                        processPNG(data, bits, precise, cb);
                    } else if ((ext === 'jpg' || ext === 'jpeg') && type.mime === 'image/jpeg') {
                        processJPG(data, bits, precise, cb);
                    } else {
    ...
    const handleReadFile = (err, res) => {
        ...
        checkFileType(src as string, res);
    };
